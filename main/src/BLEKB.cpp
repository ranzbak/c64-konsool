...
class ExeExtCmdTask {
   private:
    ...
    static void exeExternalCmd(void* parameter) {
        ...
        instance->type          = instance->externalCmds.executeExternalCmd(instance->blekb.buffer);
   ...
   public:
    ...
    uint8_t exe() {
        ...
        BaseType_t result = xTaskNotifyWait(0x00,                  // No bits to clear on entry
...
BLEKBCharacteristicCallback::BLEKBCharacteristicCallback(BLEKB& blekb, ExternalCmds& externalCmds)
...
void BLEKBCharacteristicCallback::onWrite(BLECharacteristic* pCharacteristic) {
    ...
    if (len == 1) {  // virtual joystick or release key
        ...
        if (blekb.shiftctrlcode & 128) {
            ...
            switch (type) {
                case 1:
                    pCharacteristic->setValue(reinterpret_cast<uint8_t*>(&(externalCmds.type1notification)),
                                              sizeof(BLENotificationStruct1));
                    break;
                case 2:
                    pCharacteristic->setValue(reinterpret_cast<uint8_t*>(&(externalCmds.type2notification)),
                                              sizeof(BLENotificationStruct2));
                    break;
                case 3:
                    pCharacteristic->setValue(reinterpret_cast<uint8_t*>(&(externalCmds.type3notification)),
                                              sizeof(BLENotificationStruct3));
                    break;
                case 4:
                    pCharacteristic->setValue(reinterpret_cast<uint8_t*>(&(externalCmds.type4notification)),
                                              sizeof(BLENotificationStruct4));
                    break;
                case 5:
                    pCharacteristic->setValue(reinterpret_cast<uint8_t*>(&(externalCmds.type5notification)),
...
void BLEKB::init(C64Emu* c64emu) {
    ...
    BLECharacteristic* pCharacteristic = pService->createCharacteristic(
        Config::CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY);
    pCharacteristic->setCallbacks(new BLEKBCharacteristicCallback(*this, c64emu->externalCmds));
...
void BLEKB::handleKeyPress() {
    ...
    if ((detectreleasekey && keypresseddown) || (keypresseddowncnt < NUMOFCYCLES_KEYPRESSEDDOWN)) {
        sentdc00 = buffer[0];
        sentdc01 = buffer[1];
        keypresseddowncnt++;
    } else {
...
uint8_t BLEKB::getKBJoyValue(bool port2) {
