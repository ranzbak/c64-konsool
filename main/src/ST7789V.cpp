...
esp_err_t config_lcd() {
    ...
    io_conf.pin_bit_mask = (1ULL << Config::D0) | (1ULL << Config::D1) | (1ULL << Config::D2) | (1ULL << Config::D3) |
                           (1ULL << Config::D4) | (1ULL << Config::D5) | (1ULL << Config::D6) | (1ULL << Config::D7) |
                           (1ULL << Config::CS) | (1ULL << Config::DC) | (1ULL << Config::WR) | (1ULL << Config::BL);
...
void ST7789V::init() {
    ...
    if (err != ESP_OK) {
        throw HardwareInitializationException(std::string("init. of ST7789V failed: ") + esp_err_to_name(err));
    }
...
void ST7789V::copyColor(uint16_t x0, uint16_t y0, uint16_t w, uint16_t h, uint16_t data) {
    copyinit(x0, y0, w, h);
...
void ST7789V::copyData(uint16_t x0, uint16_t y0, uint16_t w, uint16_t h, uint16_t* data) {
    copyinit(x0, y0, w, h);
    uint32_t clearMask = lu_pinbitmask[255];
    for (uint32_t i = 0; i < w * h; i++) {
        copycopy(*data++, clearMask);
...
void ST7789V::drawFrame(uint16_t frameColor) {
    if (BORDERHEIGHT > 0) {
        ...
        ST7789V::copyColor(BORDERWIDTH, 200 + BORDERHEIGHT, 320, BORDERHEIGHT, frameColor);
    }
    if (BORDERWIDTH > 0) {
        ST7789V::copyColor(0, 0, BORDERWIDTH, Config::LCDHEIGHT, frameColor);
        ST7789V::copyColor(BORDERWIDTH + 320, 0, BORDERWIDTH, Config::LCDHEIGHT, frameColor);
    }
}
...
const uint16_t* ST7789V::getC64Colors() const {
    return c64Colors;
}
#endif
